...
# ---------------------------------------------------------------------------
# phonehost: this device is a headless server, so the session comes up with the
# screen off and never suspends.
#
# The no-suspend OpenRC service holds an independent kernel wakelock, which is
# what actually blocks s2idle (sxmo_autosuspend blocks on /sys/power/wakeup_count
# while any wakelock is held). Stopping the job as well just avoids leaving it
